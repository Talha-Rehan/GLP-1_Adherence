...
export default function PatientRiskPanel() {
  ...
  const filtered = useMemo(() => {
    ...
    return d;
  }, [search, segFilter, molFilter, predFilter, minRisk, sortKey, sortDir]);

  const pages = Math.ceil(filtered.length / PAGE_SIZE);
  const visible = filtered.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE);
  const highRisk = filtered.filter(p => p.dropout_prob >= 0.75).length;
  const financial = filtered.filter(p => p.driver_1.toLowerCase().includes('financial') || p.driver_1.toLowerCase().includes('cost')).length;
  ...
  const SortBtn = ({ k, label }) => (
    <button onClick={() => toggleSort(k)} className="flex items-center gap-1 hover:text-gray-700">
      {label} {sortKey === k && <ArrowUpDown size={11} style={{ opacity: 0.6 }} />}
    </button>
  );

  return (
    <div className="max-w-[1280px] mx-auto flex gap-5 animate-fade-in">
      {/* Filter sidebar */}
      <aside className="flex-shrink-0 w-56 space-y-5">
        <div className="card p-4 space-y-4">
          <div className="font-semibold text-sm text-gray-700 flex items-center gap-2"><Filter size={14} />Filters</div>

          <div>
            <label className="text-xs text-gray-500 font-medium uppercase tracking-wider block mb-1.5">Segment</label>
          ...
          <div>
            <label className="text-xs text-gray-500 font-medium uppercase tracking-wider block mb-1.5">Molecule</label>
          ...
          <div>
            <label className="text-xs text-gray-500 font-medium uppercase tracking-wider block mb-1.5">Prediction</label>
          ...
          <div>
            <label className="text-xs text-gray-500 font-medium uppercase tracking-wider block mb-1.5">
              Min Risk Score: <span className="font-mono text-blue-600">{minRisk}%</span>
            </label>
            <input type="range" min={0} max={90} step={5} value={minRisk}
              onChange={e => { setMinRisk(+e.target.value); setPage(0); }} />
          </div>

          <button onClick={() => { setSearch(''); setSegFilter('All'); setMolFilter('All'); setPredFilter('All'); setMinRisk(0); setPage(0); }}
        ...
        <div className="card p-4 space-y-2">
          <div className="text-xs font-semibold text-gray-600 mb-2">Risk Score Legend</div>
          {[['≥75%','Critical','risk-critical'],['50–74%','High','risk-high'],['25–49%','Medium','risk-medium'],['<25%','Low','risk-low']].map(([range, label, cls]) => (
      ...
      <div className="flex-1 min-w-0 space-y-4">
        ...
        <div className="card px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-5 text-xs text-gray-600">
            ...
            <span className="text-red-600 font-medium">⚠ {highRisk} high risk (&gt;75%)</span>
            <span className="text-orange-600 font-medium">💰 {financial} financial barrier cases</span>
          </div>
        ...
        <div className="card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="data-table">
              ...
              <tbody>
                {visible.map((p) => (
                  <tr key={p.patient_idx} className="cursor-pointer" onClick={() => navigate(`/patients/${p.patient_idx}`)}>
                    ...
                    <td><span className="text-xs font-mono bg-gray-100 px-1.5 py-0.5 rounded">{p.assigned_molecule.slice(0,4)}</span></td>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
            ...
            <div className="flex items-center gap-2">
              ...
              {Array.from({ length: Math.min(5, pages) }, (_, i) => i + Math.max(0, page - 2)).filter(i => i < pages).map(i => (
                <button key={i} onClick={() => setPage(i)}
                  className="w-7 h-7 rounded-lg text-xs font-medium border transition-colors"
                  style={{ borderColor: i === page ? 'var(--color-primary)' : '#E2E8F0', background: i === page ? 'var(--color-primary)' : 'white', color: i === page ? 'white' : '#4A5568' }}>
                  {i + 1}
                </button>
              ))}
